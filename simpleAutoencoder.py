...
n = 200 #num nodes in the graph (If interested in much larger graphs a different technique must be employed. This will involve preprocessing the graphs in some way)
...
graphs = 20
data = []
numSets = 101 #This is the number of parameter values chosen + 1. Expect numSets - 1 different colors in the image plotted
for p in range(1,numSets):
    for g in range(0,graphs):
        G = nx.fast_gnp_random_graph(n, (p)/numSets)
        data.append(np.array(np.matrix.flatten(nx.adjacency_matrix(G).todense()),dtype = np.float64)[0])

#Store all the graphs in data
data = np.array(data)

#Embed all the graphs with the trained model
...
for i in range(0,len(data)):
    ...
    if counter > 20:
        counter = 1
        j = j+1
    ax.scatter(embededGraphs[i][0],embededGraphs[i][1],color=colorFader(c1,c2,j/numSets))
